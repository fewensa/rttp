...
#[test]
fn request_facade_parses_ect_metadata_without_network_policy() {
  let request =
    HttpRequest::parse(b"GET /asset HTTP/1.1\r\nHost: example.test\r\neCt: \t3G \t\r\n\r\n")
  ...
  assert_eq!(Some("3G"), request.header("ECT"));
  ...
  for value in ["", "5g", "slow-2g, 2g", "4g;foo", "\"4g\""] {
    let raw = format!("GET /asset HTTP/1.1\r\nHost: example.test\r\nECT: {value}\r\n\r\n");
    let malformed = HttpRequest::parse(raw.as_bytes()).expect("malformed ECT should be retained");
    assert!(malformed.ect().is_err(), "ECT should reject {value:?}");
    assert_eq!(Some(value), malformed.header("ECT"));
  }
  ...
  let _: HttpEctParseError = HttpEct::parse("3g\0").expect_err("control-byte ECT should fail");
