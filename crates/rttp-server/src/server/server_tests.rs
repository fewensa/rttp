...
fn request_sec_required_document_policy_parses_request_metadata_without_policy() {
  ...
  assert_eq!(
    Some("oversized-images=src;foo=bar"),
  ...
  let oversized_value =
    "x".repeat(rttp_protocol::sec_required_document_policy::MAX_DOCUMENT_POLICY_VALUE_BYTES + 1);
  let oversized = HttpRequest {
    method: "GET".to_string(),
    path: "/asset".to_string(),
    query: None,
    version: "HTTP/1.1".to_string(),
    headers: vec![HttpHeader::new("Sec-Required-Document-Policy", oversized_value)],
    body: Vec::new(),
    content_length: None,
  };
  ...
  let first = format!(
    "first={}",
    "a".repeat(rttp_protocol::sec_required_document_policy::MAX_DOCUMENT_POLICY_VALUE_BYTES / 2)
  );
  let second = format!(
    "second={}",
    "b".repeat(rttp_protocol::sec_required_document_policy::MAX_DOCUMENT_POLICY_VALUE_BYTES / 2)
  );
  let combined = HttpRequest {
    method: "GET".to_string(),
    path: "/asset".to_string(),
    ...
    version: "HTTP/1.1".to_string(),
    headers: vec![
      HttpHeader::new("Sec-Required-Document-Policy", first),
      HttpHeader::new("Sec-Required-Document-Policy", second),
    ],
    body: Vec::new(),
    content_length: None,
  };
  assert!(combined.sec_required_document_policy().is_err());
  assert!(combined.header("Sec-Required-Document-Policy").is_some());
}
