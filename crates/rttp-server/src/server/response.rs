...
pub use rttp_protocol::retry_after::{
  RetryAfter as HttpRetryAfter, RetryAfterParseError as HttpRetryAfterParseError,
};
...
impl HttpResponse {
  ...
  pub fn with_retry_after_delta(mut self, delta_seconds: u64) -> Self {
    self.headers.push(HttpHeader::new(
      "Retry-After",
      HttpRetryAfter::DeltaSeconds(delta_seconds).header_value(),
    ));
    self
  }
  ...
  pub fn with_retry_after_date(mut self, http_date: SystemTime) -> Self {
    self.headers.push(HttpHeader::new(
      "Retry-After",
      HttpRetryAfter::HttpDate(http_date).header_value(),
    ));
    self
  }
...
impl HttpRequestTe {
  pub fn parse(value: impl AsRef<str>) -> Result<Self, HttpTeParseError> {
    Self::parse_values([value.as_ref()])
  }

  pub fn parse_values<'a, I>(values: I) -> Result<Self, HttpTeParseError>
  where
    I: IntoIterator<Item = &'a str>,
  {
    let mut codings = Vec::new();
...
fn parse_te_member(member: &str) -> Result<(&str, Option<u16>), HttpTeParseError> {
  ...
  if coding.eq_ignore_ascii_case("trailers") {
    return Err(HttpTeParseError::new("TE trailers cannot carry a q-value"));
  }
  ...
  Ok((coding, Some(parse_te_qvalue(qvalue.trim())?)))
}

fn parse_te_qvalue(qvalue: &str) -> Result<u16, HttpTeParseError> {
  let Some((whole, fraction)) = qvalue.split_once('.') else {
    return match qvalue {
      "0" => Ok(0),
      "1" => Ok(1000),
      _ => Err(HttpTeParseError::new("invalid TE q-value")),
    };
  };
