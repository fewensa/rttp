...
positive ratio through `ratio()` and canonical Structured Fields decimal text
through `header_value()`. The `sf-decimal` grammar requires a decimal point,
at most 12 integer digits, and at most 3 fractional digits. Malformed or
duplicate fields return `HttpSecChDprParseError` while
`Request::header("Sec-CH-DPR")` preserves the raw value. This is
...
invalid-escape, or oversized fields return `HttpSecChUaPlatformParseError`
...
exposing canonical quoted-string text through `header_value()`. Empty,
non-string, comma-list, parameterized, duplicate, non-ASCII, forbidden-control,
invalid-escape, or oversized fields return `HttpSecChUaArchParseError` while
...
`Request::sec_ch_ua_bitness()` and `HttpRequest::sec_ch_ua_bitness()` parse a
singleton `Sec-CH-UA-Bitness` request Client Hint into `HttpSecChUaBitness`,
accepting one Structured Fields string after optional SP or HTAB trimming and
exposing canonical quoted-string text through `header_value()`. Empty,
non-string, comma-list, parameterized, duplicate, non-ASCII, forbidden-control,
invalid-escape, or oversized fields return `HttpSecChUaBitnessParseError` while
the raw field remains available. This is metadata-only: the server does not
infer an architecture, negotiate the UA brands family, emit `Accept-CH`, retry,
or apply browser policy.
...
`entries()` exposes `brand()` and `version()`/`v()` without reordering or
interpreting the values. The parser permits 256 entries and 64 KiB per field
and in aggregate, canonicalizes valid quoting, and rejects malformed,
non-ASCII/control, missing/extra/duplicate-parameter, duplicate-field,
excessive-entry, and oversized input. `Ok(None)` is returned when the field is
absent, and parse errors leave the raw field available through `header()`.
This is metadata-only: the server does not infer browser identity, negotiate
...
`Request::prefers_reduced_data()` and `HttpRequest::prefers_reduced_data()`
parse a singleton `Sec-CH-Prefers-Reduced-Data` request Client Hint into
`HttpPrefersReducedData`, accepting `no-preference` and `reduce`
case-insensitively after optional SP or HTAB trimming and exposing lowercase
canonical text through `header_value()`. Empty, unknown, comma-list, duplicate,
control-byte, non-ASCII, or oversized fields return
`HttpPrefersReducedDataParseError` while the raw field remains available. This
is metadata-only: the server does not infer a preference, adapt content,
...
overflowing, control-byte, or oversized fields return `HttpWidthParseError`
...
`Request::sec_ch_viewport_height()` and `HttpRequest::sec_ch_viewport_height()`
parse a singleton `Sec-CH-Viewport-Height` request Client Hint into
`HttpSecChViewportHeight`, exposing its non-negative integer through `value()`
and canonical text through `header_value()`. Malformed, duplicate, overflowing,
control-byte, or oversized fields return `HttpSecChViewportHeightParseError`
while the raw `Sec-CH-Viewport-Height` field remains available. These helpers
...
## Accept-Post response metadata

`HttpResponse::with_accept_post(media_types)` validates an ordered list of
media-type field values through the shared protocol `HttpAcceptPost` type,
serializes it as one `Accept-Post` field, and replaces existing fields with
that name. `HttpResponse::accept_post()` parses attached fields in wire order
into the same `HttpAcceptPost` type. The server module exposes the shared
`HttpAcceptPostParseError`, `HttpMediaType`, and `HttpMediaTypeParameter`
aliases for typed inspection.
...
invalid declaration is rejected before existing response fields are replaced,
and invalid attached fields remain available through ordinary response
serialization after `accept_post()` returns an error.
...
`HttpResponse::with_location(value)` validates one `Location` URI-reference
field value with the shared protocol-owned `HttpLocation` type, trims outer
whitespace, removes any existing raw `Location` fields, and adds a single
validated `Location` header. `HttpResponse::location()` parses attached raw
fields into `HttpLocation`, returns `Ok(None)` when absent, and preserves
...
control characters, interior whitespace, unsafe field-value characters,
malformed URI syntax, or broken percent-encoding. Duplicate fields are rejected
because `Location` is singleton response metadata. The preserved trimmed
reference is available through `as_str()` and `header_value()`.

These helpers only declare and parse metadata. RTTP does not resolve relative
references against a response URL, follow redirects, or alter status policy
from `Location`.
