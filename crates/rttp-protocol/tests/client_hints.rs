...
#[test]
fn sec_ch_ua_full_version_list_preserves_order_and_canonicalizes_strings() {
  let list = SecChUaFullVersionList::parse(r#"  "Chromium";v="120.0", "Not(A:Brand";v="99\".0"  "#)
    .expect("valid Sec-CH-UA-Full-Version-List");
  ...
  assert_eq!(
    list,
    SecChUaFullVersionList::parse(list.header_value()).expect("canonical roundtrip")
  );
}

#[test]
fn sec_ch_ua_full_version_list_rejects_malformed_duplicate_and_bounded_input() {
  assert!(SecChUaFullVersionList::parse_values([]).is_err());
  for value in [
    "",
    " ",
    r#""Chromium""#,
    r#"Chromium;v="120.0""#,
    r#""Chromium";v=120.0"#,
    r#""Chromium";v="120.0";foo="bar""#,
    r#""Chromium";foo="bar""#,
    r#""Chromium";v="120.0";v="121.0""#,
    r#""Chromium", ("Other");v="1""#,
    r#""Chromium";v="120.0", "#,
    "\"Chromium\0\";v=\"120.0\"",
    "\"Chromium\u{80}\";v=\"120.0\"",
  ] {
    assert!(
      SecChUaFullVersionList::parse(value).is_err(),
      "{value:?} must be rejected"
    );
  }

  assert!(SecChUaFullVersionList::parse_values([
    r#""Chromium";v="120.0""#,
    r#""Firefox";v="121.0""#,
  ])
  .is_err());
  let too_many = std::iter::repeat_n(
    r#""Brand";v="1""#,
    MAX_SEC_CH_UA_FULL_VERSION_LIST_ENTRIES + 1,
  )
  .collect::<Vec<_>>()
  .join(",");
  assert!(SecChUaFullVersionList::parse(&too_many).is_err());
  assert!(SecChUaFullVersionList::parse(format!(
    r#""{}";v="1""#,
    "x".repeat(MAX_SEC_CH_UA_FULL_VERSION_LIST_VALUE_BYTES)
  ))
  .is_err());
  let aggregate_member = format!(
    r#""{}";v="1""#,
    "x".repeat(MAX_SEC_CH_UA_FULL_VERSION_LIST_TOTAL_BYTES / 2)
  );
  assert!(SecChUaFullVersionList::parse_values([
...
#[test]
fn sec_ch_viewport_height_parses_non_negative_integer_and_round_trips() {
  for (value, expected) in [
    ("0", 0),
    ("1", 1),
    ("900", 900),
    ...
    assert_eq!(value, viewport_height.header_value());
    assert_eq!(
      viewport_height,
      SecChViewportHeight::parse(viewport_height.header_value())
        .expect("Sec-CH-Viewport-Height roundtrip")
    );
    assert_eq!(viewport_height, SecChViewportHeight::new(expected));
  }
}

#[test]
fn sec_ch_viewport_height_trims_outer_optional_whitespace() {
  let viewport_height =
    SecChViewportHeight::parse("\t 900 \t").expect("OWS-padded Sec-CH-Viewport-Height");
  assert_eq!(900, viewport_height.value());
  assert_eq!("900", viewport_height.header_value());
}

#[test]
fn sec_ch_viewport_height_rejects_malformed_duplicate_empty_and_overflow_values() {
  assert!(SecChViewportHeight::parse_values(["1", "2"]).is_err());
...
#[test]
fn sec_ch_viewport_height_rejects_oversized_and_control_byte_values() {
  assert!(
    SecChViewportHeight::parse("1".repeat(MAX_SEC_CH_VIEWPORT_HEIGHT_VALUE_BYTES + 1)).is_err()
  );
  assert!(SecChViewportHeight::parse("1\r\nInjected: yes").is_err());
  assert!(SecChViewportHeight::parse("1\u{7f}").is_err());
}
