...
#[test]
fn sec_required_document_policy_rejects_malformed_control_and_duplicate_values() {
  for value in [
    "",
    "   ",
    "oversized-images=()",
    ...
    "oversized-images=2.0\n, unsized-media=?0",
    "oversized-images=2.0\0, unsized-media=?0",
  ] {
    assert!(
...
fn sec_required_document_policy_enforces_shared_size_and_member_bounds() {
  ...
  let at_limit = (0..MAX_DOCUMENT_POLICY_DIRECTIVES)
    ...
    .collect::<Vec<_>>()
    .join(", ");
  let parsed = SecRequiredDocumentPolicy::parse(&at_limit).expect("256 directives should parse");
