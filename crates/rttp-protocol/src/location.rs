...
//! fragment, query, or userinfo.
//!
//! Each field is bounded to 64 KiB. ASCII controls other than HTAB, empty
//! values, duplicate fields, interior whitespace, non-URI bytes, broken
//! percent-encoding, and values the structural URL parser cannot accept as an
//! absolute or relative reference are errors. Exotic RFC 3986 forms that the
//! `url` crate rejects are treated as malformed.
//!
//! Parsing is response metadata validation only. It does not follow redirects,
//! resolve relatives against a request URL, replay requests, rewrite methods,
//! strip headers, or make trust decisions for callers.
