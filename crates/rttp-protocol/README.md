...
`accept_post` parses one or more `Accept-Post` response field values into an
ordered list of `MediaType` values. Repeated fields are flattened in wire
order, and duplicate media types are retained because this protocol primitive
does not impose application policy. Each field value is limited to 64 KiB and
the combined parsed list is limited to 256 media types.
...
`parameters()`. Media-type tokens, optional whitespace, quoted parameter values
(including commas and escaped quotes), and control-byte rejection are validated
by the shared parser. `AcceptPost::header_value()` serializes the validated
list with canonical separators and quoted-string escaping. Empty values,
malformed media types or parameters, forbidden control bytes, oversized field
values, and more than 256 members return `AcceptPostParseError`.

This type reports response metadata only. It does not route `POST` requests,
decode payloads, negotiate media types, or select a method automatically.
...
Hint as a finite positive Structured Fields decimal (`sf-decimal`) ratio. The
field value is bounded to 64 KiB, optional outer SP and HTAB are trimmed, and
the accepted value is serialized canonically by `header_value()`. Empty,
malformed, duplicate, non-finite, non-positive, control-byte, and oversized
values are errors; the Structured Fields decimal grammar requires a decimal
point, at most 12 integer digits, and at most 3 fractional digits. This parser
...
`client_hints::SecChUaWow64` parses a singleton HTTP `Sec-CH-UA-WoW64`
request Client Hint as exactly one RFC 8941 boolean item, the tokens `?0` or
`?1`. The field value is bounded to 64 KiB, optional outer SP and HTAB are
trimmed, and `header_value()` emits the canonical `?0` / `?1` tokens. Empty,
unknown, comma-list, parameterized, duplicate, non-ASCII, forbidden
control-byte, and oversized values are errors. This parser reports request
metadata only; it does not infer platform architecture, negotiate the UA brands
family, emit `Accept-CH`, retry, or apply browser policy.

## Sec-CH-UA-Platform

`client_hints::SecChUaPlatform` parses a singleton HTTP `Sec-CH-UA-Platform`
request Client Hint as one bounded Structured Fields string. The field value is
bounded to 64 KiB, optional outer SP and HTAB are trimmed, and
`header_value()` emits canonical quoted-string syntax with quote and backslash
escapes. `value()` exposes the decoded ASCII string. Empty, non-string,
comma-list, parameterized, duplicate, non-ASCII, forbidden-control, invalid
escape, and oversized values are errors. This parser reports request metadata
only; it does not infer a platform, negotiate the UA brands family, emit
`Accept-CH`, retry, or apply browser policy.

## Sec-CH-UA-Model

`client_hints::SecChUaModel` parses a singleton HTTP `Sec-CH-UA-Model`
request Client Hint as one bounded RFC 8941 Structured Fields string. The field
value is bounded to 64 KiB, optional outer SP and HTAB are trimmed, and
`header_value()` emits canonical quoted-string syntax with quote and backslash
escapes. `value()` exposes the decoded ASCII string. Empty, non-string,
comma-list, parameterized, duplicate, non-ASCII, forbidden-control, invalid
escape, and oversized values are errors. This parser reports metadata only; it
does not infer device identity, negotiate the UA brands family, emit
`Accept-CH`, retry, or apply browser policy.

## Sec-CH-UA-Arch

`client_hints::SecChUaArch` parses a singleton HTTP `Sec-CH-UA-Arch`
request Client Hint as one bounded Structured Fields string. The field value is
bounded to 64 KiB, optional outer SP and HTAB are trimmed, and
`header_value()` emits canonical quoted-string syntax with quote and backslash
escapes. `value()` exposes the decoded ASCII string. Empty, non-string,
comma-list, parameterized, duplicate, non-ASCII, forbidden-control, invalid
escape, and oversized values are errors. This parser reports request metadata
only; it does not infer an architecture, negotiate the UA brands family, emit
`Accept-CH`, retry, or apply browser policy.

## Sec-CH-UA-Bitness

`client_hints::SecChUaBitness` parses a singleton HTTP `Sec-CH-UA-Bitness`
request Client Hint as one bounded Structured Fields string. The field value is
bounded to 64 KiB, optional outer SP and HTAB are trimmed, and
`header_value()` emits canonical quoted-string syntax with quote and backslash
escapes. `value()` exposes the decoded ASCII string. Empty, non-string,
...
`client_hints::SecChUaPlatformVersion` parses a singleton HTTP
`Sec-CH-UA-Platform-Version` request Client Hint as one bounded Structured
Fields string. The field value is bounded to 64 KiB, optional outer SP and HTAB
are trimmed, and `header_value()` emits canonical quoted-string syntax with
quote and backslash escapes. `value()` exposes the decoded ASCII string. Empty,
...
`client_hints::PrefersReducedData` parses a singleton HTTP
`Sec-CH-Prefers-Reduced-Data` request Client Hint as the `no-preference` or
`reduce` token. The field value is bounded to 64 KiB, optional outer SP and
HTAB are trimmed, tokens are matched case-insensitively, and `header_value()`
emits lowercase canonical text. Empty, unknown, comma-list, duplicate,
forbidden control-byte, non-ASCII, and oversized values are errors. This
parser reports request metadata only; it does not infer a preference, adapt
content, negotiate `Accept-CH`, retry, or apply browser policy.

## Sec-CH-Prefers-Contrast

`client_hints::PrefersContrast` parses a singleton HTTP
`Sec-CH-Prefers-Contrast` request Client Hint as the `no-preference`, `more`,
`less`, or `custom` token. The field value is bounded to 64 KiB, optional outer
SP and HTAB are trimmed, tokens are matched case-insensitively, and
`header_value()` emits lowercase canonical text. Empty, unknown, comma-list,
duplicate, forbidden control-byte, and oversized values are errors. This parser
reports request metadata only; it does not infer a preference, adapt content,
negotiate `Accept-CH`, retry, or apply browser policy.
...
`client_hints::ViewportWidth` parses a singleton HTTP `Viewport-Width` request
Client Hint as a non-negative unsigned integer. The field value is bounded to
64 KiB, optional outer SP and HTAB are trimmed, and `header_value()` emits
canonical decimal text. Empty, malformed, duplicate, signed, fractional,
overflowing, control-byte, and oversized values are errors. This parser reports
request metadata only; it does not negotiate content, emit Client Hints, track
viewport size, or adapt representations.

## Sec-CH-Viewport-Height

`client_hints::SecChViewportHeight` parses a singleton HTTP
`Sec-CH-Viewport-Height` request Client Hint as a non-negative Structured
Fields integer from 0 through 999999999999999. The field value is bounded to 64
KiB, optional outer SP and HTAB are trimmed, and `header_value()` emits
canonical decimal text. Empty, malformed, duplicate, signed, fractional,
out-of-range, overflowing, control-byte, and oversized values are errors. This
parser reports request metadata only; it does not negotiate content, emit
Client Hints, track viewport size, or adapt representations.

## Sec-CH-Viewport-Width

`client_hints::SecChViewportWidth` parses a singleton HTTP
`Sec-CH-Viewport-Width` request Client Hint as a non-negative Structured
Fields integer from 0 through 999999999999999. The field value is bounded to
64 KiB, optional outer SP and HTAB are trimmed, and `header_value()` emits
canonical decimal text. Empty, malformed, duplicate, signed, fractional,
parameterized, comma-list, out-of-range, overflowing, non-ASCII, control-byte,
and oversized values are errors. This parser reports request metadata only; it
does not negotiate content, emit Client Hints, track viewport size, or adapt
representations.

## RTT

`client_hints::Rtt` parses a singleton HTTP `RTT` request Client Hint as a
non-negative unsigned millisecond integer. The field value is bounded to 64 KiB,
optional outer SP and HTAB are trimmed, and `header_value()` emits canonical
decimal text. Empty, malformed, duplicate, signed, fractional, overflowing,
control-byte, and oversized values are errors. This parser reports request
metadata only; it does not negotiate content, emit Client Hints, measure
network delay, or adapt representations.
