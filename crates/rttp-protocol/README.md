...
Client Hint as one bounded Structured Fields string. The field value is
bounded to 64 KiB, optional outer SP and HTAB are trimmed, and
`header_value()` emits canonical quoted-string syntax with quote and backslash
escapes. `value()` exposes the decoded ASCII architecture string. Empty,
non-string, comma-list, parameterized, duplicate, non-ASCII, forbidden-control,
invalid escape, and oversized values are errors. This parser reports
architecture metadata only; it does not infer a platform, negotiate the UA
brands family, emit `Accept-CH`, retry, or apply browser policy. Callers that
need an intentionally unvalidated wire value can use the raw header API.
