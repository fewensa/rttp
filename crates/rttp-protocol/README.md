...
original quoted-string quoting. This type is the shared authority for token,
parameter, duplicate, member-count, and size validation. It reports declared
response metadata only; it does not select, invert, decode, or apply instance
manipulations and does not impose a `226 IM Used` status policy.
