use rttp::server::{
  ...
  HttpSecChUaFullVersionListParseError, HttpSecChUaFullVersionParseError, HttpSecChUaMobile,
...
#[test]
fn compatibility_facade_exports_rtt_request_metadata() {
  let request =
    HttpRequest::parse(b"GET /asset HTTP/1.1\r\nHost: example.test\r\nRTT: \t150 \t\r\n\r\n")
...
#[test]
fn compatibility_facade_exports_sec_ch_ua_mobile_request_metadata() {
  let request = HttpRequest::parse(
    b"GET /asset HTTP/1.1\r\nHost: example.test\r\nSec-CH-UA-Mobile: \t?1 \t\r\n\r\n",
  )
...
#[test]
fn compatibility_facade_exports_sec_ch_ua_bitness_request_metadata() {
  let request = HttpRequest::parse(
  ...
  assert_eq!(
    None,
    absent.sec_ch_ua_bitness().expect("absence should be valid")
  );
...
#[test]
fn compatibility_facade_exports_sec_ch_ua_request_metadata() {
  let request = HttpRequest::parse(
...
#[test]
fn compatibility_facade_exports_sec_ch_ua_full_version_request_metadata() {
  let request = HttpRequest::parse(
  ...
  assert_eq!(
    ...
    request.header("Sec-CH-UA-Full-Version")
  );
  ...
  .expect("duplicate Sec-CH-UA-Full-Version fields should remain parseable");
...
#[test]
fn compatibility_facade_exports_downlink_request_metadata() {
  let request =
    HttpRequest::parse(b"GET /asset HTTP/1.1\r\nHost: example.test\r\nDownlink: \t1.5 \t\r\n\r\n")
  ...
  assert_eq!(None, absent.downlink().expect("absence should be valid"));

  let malformed =
    HttpRequest::parse(b"GET /asset HTTP/1.1\r\nHost: example.test\r\nDownlink: 1e1\r\n\r\n")
      .expect("malformed Downlink should remain available");
  let _: HttpDownlinkParseError = malformed
    .downlink()
    .expect_err("malformed Downlink should fail");
