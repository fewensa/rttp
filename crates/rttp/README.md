...
exposes canonical lowercase text for the standard `slow-2g`, `2g`, `3g`, or
`4g` token, while RFC 5234 token matching accepts ASCII case variants. Parse
errors leave the raw header available. These helpers only expose metadata:
RTTP does not infer network quality, negotiate content, emit `Accept-CH`,
retry, or apply Client Hints policy.
