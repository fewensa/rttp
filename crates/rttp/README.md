...
| Metadata HTTP/1.1 interoperability matrix | Workspace integration tests cover live HTTP/1.1 client/server/facade roundtrips for `From`, `Referer`, `DPR`, `ECT`, `Accept-Patch`, `Accept-Post`, and `RateLimit-*`, including canonical wire values, ordered typed values, raw escape hatches, absence as `Ok(None)`, malformed peer input with raw preservation, pre-connect builder rejection, and sync/async client parity | No identity, referrer, method-negotiation, quota, or network-condition policy |
