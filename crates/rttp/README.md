...
The facade exposes `HttpResponse::with_accept_post(media_types)` and
`HttpResponse::accept_post()` through `rttp::server`. The declaration helper
joins and validates an ordered list of media-type values with the shared
`HttpAcceptPost` protocol type, serializes one `Accept-Post` field, and
replaces existing same-name fields. The accessor combines repeated fields in
wire order and returns the shared `HttpAcceptPost` metadata type. The server
aliases `HttpAcceptPostParseError`, `HttpMediaType`, and
`HttpMediaTypeParameter` are available for typed inspection.
...
declarations fail before replacement, and invalid attached values leave raw
headers available through ordinary response serialization. The facade also
re-exports the client `AcceptPost` and `AcceptPostParseError` types behind its
`client` feature, where `Response::accept_post()` provides the same behavior.

This metadata is informational only. The facade does not route `POST`
requests, decode payloads, negotiate media types, select a method, retry, or
...
`Request::range(entity_length)` / `HttpRequest::range(entity_length)` or
`HttpByteRangeSet::parse(range_header, entity_length)`.

Those request helpers resolve one `bytes` field into `HttpByteRangeSet`: closed
`bytes=start-end`, open-ended `bytes=start-`, and suffix `bytes=-suffix`
members in one header, in wire order, using checked arithmetic. The set is
never empty and contains at most 32 members (`HttpByteRangeSet::MAX_RANGES`).
Unsatisfiable members are omitted; if every member is unsatisfiable the result
...
inverted members return `InvalidRange`.
...
The helper returns `PartialContent(HttpByteRangeSet)` when the request can use
the resolved range set after a successful validator check,
`RangeNotSatisfiable` when every guarded member is outside the representation,
or `FullResponse` when there is no `Range` header or an `If-Range` validator is
absent, invalid, weak, stale, or missing from the caller-provided metadata.
...
With the client feature, `HttpClient::sec_ch_dpr(value)` validates and emits
...
emits one singleton `Sec-CH-UA-Platform` request field through
`rttp::SecChUaPlatform`, replacing any existing case-insensitive field. The
...
server facade, `Request::sec_ch_ua_platform()` and
`HttpRequest::sec_ch_ua_platform()` parse received fields into
`HttpSecChUaPlatform`; parse errors leave the raw header available. Empty,
non-string, comma-list, parameterized, duplicate, non-ASCII, forbidden-control,
invalid-escape, and oversized values are rejected. These helpers only expose
...
With the client feature, `HttpClient::sec_ch_ua_model(value)` validates and
emits one singleton `Sec-CH-UA-Model` request Client Hint through
`rttp::SecChUaModel`, replacing any existing case-insensitive field. The value
must be one RFC 8941 Structured Fields string; optional surrounding SP or HTAB
is trimmed and `header_value()` emits canonical quoted-string syntax. On the
server facade, `Request::sec_ch_ua_model()` and `HttpRequest::sec_ch_ua_model()`
parse received fields into `HttpSecChUaModel`; parse errors leave the raw header
...
helpers only expose metadata: RTTP does not infer device identity, negotiate
...
emits one singleton `Sec-CH-UA-Arch` request field through
`rttp::SecChUaArch`, replacing any existing case-insensitive field. The value
must be one Structured Fields string; optional surrounding SP or HTAB is
trimmed and `header_value()` emits canonical quoted-string syntax. On the
server facade, `Request::sec_ch_ua_arch()` and `HttpRequest::sec_ch_ua_arch()`
parse received fields into `HttpSecChUaArch`; parse errors leave the raw header
available. Empty, non-string, comma-list, parameterized, duplicate, non-ASCII,
forbidden-control, invalid-escape, and oversized values are rejected. These
...
With the client feature, `HttpClient::sec_ch_ua_platform_version(value)`
validates and emits one singleton `Sec-CH-UA-Platform-Version` request field
through `rttp::SecChUaPlatformVersion`, replacing any existing case-insensitive
field. The value must be one Structured Fields string; optional surrounding SP
or HTAB is trimmed and `header_value()` emits canonical quoted-string syntax.
On the server facade, `Request::sec_ch_ua_platform_version()` and
`HttpRequest::sec_ch_ua_platform_version()` parse received fields into
`HttpSecChUaPlatformVersion`; parse errors leave the raw header available.
Empty, non-string, comma-list, parameterized, duplicate, non-ASCII,
forbidden-control, invalid-escape, and oversized values are rejected. These
helpers only expose metadata: RTTP does not infer a platform version or
...
value must be one Structured Fields string; optional surrounding SP or HTAB is
trimmed and `header_value()` emits canonical quoted-string syntax. On the
server facade, `Request::sec_ch_ua_full_version()` and
`HttpRequest::sec_ch_ua_full_version()` parse received fields into
`HttpSecChUaFullVersion`; parse errors leave the raw header available. Empty,
non-string, comma-list, parameterized, duplicate, non-ASCII, forbidden-control,
invalid-escape, and oversized values are rejected. These helpers only expose
metadata: RTTP does not infer browser or platform versions, negotiate the UA
brands family, emit `Accept-CH`, retry, or apply browser policy. Use
`header(("Sec-CH-UA-Full-Version", "..."))` as the raw-header escape hatch.
...
With the client feature, `HttpClient::prefers_reduced_data(value)` validates
and emits one singleton `Sec-CH-Prefers-Reduced-Data` request field through
`rttp::PrefersReducedData`, replacing any existing case-insensitive field.
The `no-preference` and `reduce` tokens are parsed case-insensitively after
surrounding SP or HTAB trimming and `header_value()` emits lowercase canonical
text. On the server facade, `Request::prefers_reduced_data()` and
`HttpRequest::prefers_reduced_data()` parse received fields into
`HttpPrefersReducedData`; parse errors leave the raw header available.
Empty, unknown, comma-list, duplicate, control-byte, non-ASCII, and oversized
values are rejected. These helpers only expose metadata: RTTP does not infer a
preference, adapt content, negotiate `Accept-CH`, retry, or apply browser
policy.
...
case-insensitive field. Recognized tokens are `slow-2g`, `2g`, `3g`, and
`4g`; surrounding SP or HTAB is trimmed and `header_value()` emits the
canonical token. On the server facade, `Request::ect()` and
`HttpRequest::ect()` parse received fields into `HttpEct`. Parse errors leave
the raw header available. These helpers only expose metadata: RTTP does not
infer network conditions, emit `Accept-CH`, persist Client Hints policy,
retry, or generate Client Hints automatically.
...
With the client feature, `HttpClient::sec_ch_viewport_height(value)` validates
and emits one singleton `Sec-CH-Viewport-Height` request field through
`rttp::SecChViewportHeight`, replacing any existing case-insensitive field. On
the server facade, `Request::sec_ch_viewport_height()` and
`HttpRequest::sec_ch_viewport_height()` parse received fields into
`HttpSecChViewportHeight`; `value()` and `header_value()` expose the typed and
...
`SecRequiredDocumentPolicy` type, replacing any existing same-name field with
the canonical value before connecting. Server-side
`Request::sec_required_document_policy()` and
`HttpRequest::sec_required_document_policy()` parse the same Document Policy
Structured Fields dictionary model and bounds into
`HttpSecRequiredDocumentPolicy`, returning `Ok(None)` when absent.
Malformed, control-bearing, duplicate, or oversized values return a parser
error while raw request headers remain available.

These helpers only declare or parse request metadata. RTTP does not enforce
document policy, compare values against `Document-Policy`, block document
loads, or send reports.
...
| Accept-Post | `HttpResponse::with_accept_post` and `HttpResponse::accept_post` declare or parse repeated bounded response fields through shared `HttpAcceptPost`/`HttpMediaType` types, replacing raw fields on valid declaration and preserving them on parse errors; client `Response::accept_post` exposes the same protocol `AcceptPost` metadata | No POST routing, payload decoding, media-type negotiation, method selection, retry, or automatic follow-up request |
...
| Client Hints | `HttpAcceptCh`/`HttpCriticalCh`, `HttpResponse::with_accept_ch`/`with_critical_ch`, and `accept_ch`/`critical_ch` declare and parse bounded Client Hints opt-in metadata; `Response::accept_ch` and `critical_ch` expose it to clients; `HttpClient::ect`, `Request::ect`, and `HttpRequest::ect` share bounded singleton `ECT` request tokens `slow-2g`, `2g`, `3g`, and `4g` while preserving raw headers on errors | No browser opt-in state, request-header generation, automatic retry, persistence, Client Hints policy, or network-condition inference |
...
| HTTP message signatures | `HttpClient::signature`, `signature_input`, and `accept_signature` emit bounded RFC 9421 request metadata; `Request`/`HttpRequest` parse received signature fields; `HttpResponse::with_accept_signature`/`accept_signature` and client `Response::accept_signature` expose bounded `Accept-Signature` metadata while preserving raw headers on parse failures | No signing, verification, key lookup, covered-component canonicalization, cryptographic policy, or signature negotiation |
...
| Metadata HTTP/1.1 interoperability matrix | Workspace integration tests cover live HTTP/1.1 client/server/facade roundtrips for `From`, `Referer`, `DPR`, `Sec-CH-DPR`, `ECT`, `Accept-Patch`, `Accept-Post`, and `RateLimit-*`, including canonical wire values, ordered typed values, raw escape hatches, absence as `Ok(None)`, malformed peer input with raw preservation, pre-connect builder rejection, and sync/async client parity | No identity, referrer, method-negotiation, quota, viewport inference, or network-condition policy |
