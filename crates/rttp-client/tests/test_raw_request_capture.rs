...
fn authorization_helper_rejects_non_ows_scheme_padding_before_connecting() {
  for whitespace in ["\u{000b}", "\u{000c}", "\r", "\n", "\u{00a0}", "\u{2003}"] {
    ...
    let credentials = "token-123".to_string();
    let request = capture_optional_request(|base_url| {
      let mut client = client();
      let error = client
        .get()
        .url(format!("{}/asset", base_url))
        .authorization(&scheme, &credentials)
        .expect_err("non-OWS Authorization scheme padding should be rejected");
      assert!(error.is_builder());
      assert!(!error.to_string().contains(&credentials));
    });
    assert!(
      request.is_empty(),
      "non-OWS Authorization scheme padding should not open a socket"
    );
  }
}
...
fn downlink_helper_emits_one_canonical_request_client_hint() {
  ...
  assert_eq!(Some("1.5"), header_value(&request, "Downlink"));
  assert_eq!(
    1,
    request
      .lines()
      .filter(|line| line.to_ascii_lowercase().starts_with("downlink:"))
      .count(),
    "typed Downlink should replace an existing same-name field"
  );
}

#[test]
fn downlink_helper_rejects_malformed_values_before_connecting() {
  let oversized = "1".repeat(64 * 1024 + 1);
  for value in [
    "",
    "-0",
    ...
    let request = capture_optional_request(|base_url| {
      let error = client()
        ...
        .url(format!("{}/asset", base_url))
        .downlink(value)
        .expect_err("invalid Downlink input must be rejected");
      assert!(error.is_builder());
    });
    assert!(
      request.is_empty(),
      "invalid Downlink input must not open a socket"
    );
  }
}

#[test]
fn raw_downlink_header_remains_available_as_escape_hatch() {
  let request = capture_request(|base_url| {
    client()
      .get()
      .url(format!("{}/asset", base_url))
      .header(("Downlink", "legacy-token"))
      .emit()
      .expect("request should succeed");
  });
  let request = request_text(&request);

  assert_eq!(Some("legacy-token"), header_value(&request, "Downlink"));
}
...
#[test]
fn sec_ch_ua_model_helper_emits_one_canonical_request_client_hint() {
  let request = capture_request(|base_url| {
    client()
      .get()
      .url(format!("{}/asset", base_url))
      .header(("sec-ch-ua-model", "legacy"))
      .sec_ch_ua_model("\t\"Pixel 8\" \t")
      .expect("Sec-CH-UA-Model should be accepted")
      .emit()
      .expect("request should succeed");
  });
  let request = request_text(&request);
  ...
  assert_eq!(
    Some(r#""Pixel 8""#),
    header_value(&request, "Sec-CH-UA-Model")
  );
  assert_eq!(
    1,
    request
      .lines()
      .filter(|line| { line.to_ascii_lowercase().starts_with("sec-ch-ua-model:") })
      .count(),
    "typed Sec-CH-UA-Model should replace an existing same-name field"
  );
}

#[cfg(feature = "async")]
#[test]
fn async_sec_ch_ua_model_helper_matches_blocking_contract() {
  for (value, expected) in [
    ("\t\"Pixel 8\" \t", r#""Pixel 8""#),
    ("\t\"Galaxy S24\" \t", r#""Galaxy S24""#),
  ] {
    let request = capture_request(|base_url| {
      block_on(
        client()
    ...
    assert_eq!(Some(expected), header_value(&request, "Sec-CH-UA-Model"));
    assert_eq!(
      1,
      request
        .lines()
...
#[test]
fn sec_ch_ua_model_helper_rejects_malformed_values_before_connecting() {
  let oversized = format!("\"{}\"", "x".repeat(64 * 1024));
  for value in [
    "",
    " ",
    ...
    let request = capture_optional_request(|base_url| {
      let error = client()
        ...
        .url(format!("{}/asset", base_url))
        .sec_ch_ua_model(value)
        .expect_err("invalid Sec-CH-UA-Model input must be rejected");
      assert!(error.is_builder());
    });
    assert!(
      request.is_empty(),
      "invalid Sec-CH-UA-Model input must not open a socket"
    );
  }
}
...
#[cfg(feature = "async")]
#[test]
fn async_sec_ch_ua_model_helper_rejects_malformed_values_before_connecting() {
  let oversized = format!("\"{}\"", "x".repeat(64 * 1024));
  for value in [
    "",
    " ",
    ...
    let request = capture_optional_request(|base_url| {
      let error = block_on(async {
        let mut http_client = client();
        let request = http_client.get().url(format!("{}/asset", base_url));
        request.sec_ch_ua_model(value)?.rasync().await
      })
      .expect_err("invalid Sec-CH-UA-Model input must be rejected");
      assert!(error.is_builder());
    });
...
#[test]
fn raw_sec_ch_ua_model_header_remains_available_as_escape_hatch() {
  let request = capture_request(|base_url| {
    client()
      .get()
      .url(format!("{}/asset", base_url))
      .header(("Sec-CH-UA-Model", "legacy-token"))
      .emit()
      .expect("request should succeed");
  });
  let request = request_text(&request);
...
#[test]
fn sec_ch_ua_arch_helper_emits_one_canonical_request_client_hint() {
  let request = capture_request(|base_url| {
    client()
      .get()
      .url(format!("{}/asset", base_url))
      .header(("sec-ch-ua-arch", "legacy"))
      .sec_ch_ua_arch("\t\"x86\\\\\\\"\" \t")
  ...
  assert_eq!(
    Some(r#""x86\\\"""#),
    header_value(&request, "Sec-CH-UA-Arch")
  );
  assert_eq!(
    1,
    request
      .lines()
      .filter(|line| { line.to_ascii_lowercase().starts_with("sec-ch-ua-arch:") })
      .count(),
    "typed Sec-CH-UA-Arch should replace an existing same-name field"
  );
}

#[cfg(feature = "async")]
#[test]
fn async_sec_ch_ua_arch_helper_matches_blocking_contract() {
  for (value, expected) in [
    ("\t\"x86\" \t", r#""x86""#),
    ("\t\"arm64\" \t", r#""arm64""#),
  ] {
    ...
    assert_eq!(Some(expected), header_value(&request, "Sec-CH-UA-Arch"));
    assert_eq!(
      1,
      request
        .lines()
...
#[test]
fn sec_ch_ua_arch_helper_rejects_malformed_values_before_connecting() {
  let oversized = format!("\"{}\"", "x".repeat(64 * 1024));
  for value in [
    "",
    " ",
    ...
    let request = capture_optional_request(|base_url| {
      let error = client()
        ...
        .url(format!("{}/asset", base_url))
        .sec_ch_ua_arch(value)
        .expect_err("invalid Sec-CH-UA-Arch input must be rejected");
      assert!(error.is_builder());
    });
    assert!(
      request.is_empty(),
      "invalid Sec-CH-UA-Arch input must not open a socket"
    );
  }
}
...
#[cfg(feature = "async")]
#[test]
fn async_sec_ch_ua_arch_helper_rejects_malformed_values_before_connecting() {
  let oversized = format!("\"{}\"", "x".repeat(64 * 1024));
  for value in [
    "",
    " ",
    ...
    let request = capture_optional_request(|base_url| {
      let error = block_on(async {
        let mut http_client = client();
        let request = http_client.get().url(format!("{}/asset", base_url));
        request.sec_ch_ua_arch(value)?.rasync().await
      })
      .expect_err("invalid Sec-CH-UA-Arch input must be rejected");
      assert!(error.is_builder());
    });
...
#[test]
fn raw_sec_ch_ua_arch_header_remains_available_as_escape_hatch() {
  let request = capture_request(|base_url| {
    client()
      .get()
      .url(format!("{}/asset", base_url))
      .header(("Sec-CH-UA-Arch", "legacy-token"))
      .emit()
      .expect("request should succeed");
  });
  let request = request_text(&request);
...
#[test]
fn sec_ch_ua_bitness_helper_emits_one_canonical_request_client_hint() {
  let request = capture_request(|base_url| {
    client()
      .get()
      .url(format!("{}/asset", base_url))
      .header(("sec-ch-ua-bitness", "legacy"))
      .sec_ch_ua_bitness("\t\"64\\\\\\\"\" \t")
      .expect("Sec-CH-UA-Bitness should be accepted")
      .emit()
      .expect("request should succeed");
  });
  let request = request_text(&request);
  ...
  assert_eq!(
    Some(r#""64\\\"""#),
    header_value(&request, "Sec-CH-UA-Bitness")
  );
  assert_eq!(
    1,
    request
      .lines()
      .filter(|line| { line.to_ascii_lowercase().starts_with("sec-ch-ua-bitness:") })
      .count(),
    "typed Sec-CH-UA-Bitness should replace an existing same-name field"
  );
}

#[cfg(feature = "async")]
#[test]
fn async_sec_ch_ua_bitness_helper_matches_blocking_contract() {
  for (value, expected) in [("\t\"64\" \t", r#""64""#), ("\t\"32\" \t", r#""32""#)] {
    let request = capture_request(|base_url| {
      block_on(
        client()
          .get()
    ...
    assert_eq!(Some(expected), header_value(&request, "Sec-CH-UA-Bitness"));
    assert_eq!(
      1,
      request
        .lines()
...
#[test]
fn sec_ch_ua_bitness_helper_rejects_malformed_values_before_connecting() {
  let oversized = format!("\"{}\"", "x".repeat(64 * 1024));
  for value in [
    "",
    " ",
    ...
    let request = capture_optional_request(|base_url| {
      let error = client()
        ...
        .url(format!("{}/asset", base_url))
        .sec_ch_ua_bitness(value)
        .expect_err("invalid Sec-CH-UA-Bitness input must be rejected");
      assert!(error.is_builder());
    });
    assert!(
      request.is_empty(),
      "invalid Sec-CH-UA-Bitness input must not open a socket"
    );
  }
}
...
#[cfg(feature = "async")]
#[test]
fn async_sec_ch_ua_bitness_helper_rejects_malformed_values_before_connecting() {
  let oversized = format!("\"{}\"", "x".repeat(64 * 1024));
  for value in [
    "",
    " ",
    ...
    let request = capture_optional_request(|base_url| {
      let error = block_on(async {
        let mut http_client = client();
        let request = http_client.get().url(format!("{}/asset", base_url));
        request.sec_ch_ua_bitness(value)?.rasync().await
      })
      .expect_err("invalid Sec-CH-UA-Bitness input must be rejected");
      assert!(error.is_builder());
    });
...
#[test]
fn raw_sec_ch_ua_bitness_header_remains_available_as_escape_hatch() {
  let request = capture_request(|base_url| {
    client()
      .get()
      .url(format!("{}/asset", base_url))
      .header(("Sec-CH-UA-Bitness", "legacy-token"))
      .emit()
      .expect("request should succeed");
  });
  let request = request_text(&request);
...
fn prefers_reduced_motion_helper_emits_one_canonical_request_client_hint() {
  let request = capture_request(|base_url| {
    client()
      .get()
      .url(format!("{}/asset", base_url))
      .header(("sec-ch-prefers-reduced-motion", "legacy"))
      .prefers_reduced_motion("\tReDuCe \t")
      .expect("Sec-CH-Prefers-Reduced-Motion should be accepted")
      .emit()
      .expect("request should succeed");
  });
  let request = request_text(&request);
  ...
  assert_eq!(
    Some("reduce"),
    header_value(&request, "Sec-CH-Prefers-Reduced-Motion")
  );
  assert_eq!(
    1,
    request
      ...
      .filter(|line| {
        line
          .to_ascii_lowercase()
          .starts_with("sec-ch-prefers-reduced-motion:")
      })
      .count(),
    "typed Sec-CH-Prefers-Reduced-Motion should replace an existing same-name field"
  );
}

#[test]
fn prefers_reduced_motion_helper_rejects_malformed_values_before_connecting() {
  let oversized = "a".repeat(64 * 1024 + 1);
  for value in [
    "",
    " ",
    ...
    let request = capture_optional_request(|base_url| {
      let error = client()
        ...
        .expect_err("invalid Sec-CH-Prefers-Reduced-Motion input must be rejected");
      assert!(error.is_builder());
    });
    assert!(
      request.is_empty(),
      "invalid Sec-CH-Prefers-Reduced-Motion input must not open a socket"
    );
  }
}

#[test]
fn raw_prefers_reduced_motion_header_remains_available_as_escape_hatch() {
  let request = capture_request(|base_url| {
    client()
      .get()
      .url(format!("{}/asset", base_url))
      .header(("Sec-CH-Prefers-Reduced-Motion", "legacy-token"))
      .emit()
      .expect("request should succeed");
  });
  let request = request_text(&request);
...
#[test]
fn prefers_reduced_transparency_helper_emits_one_canonical_request_client_hint() {
  let request = capture_request(|base_url| {
    client()
      .get()
      .url(format!("{}/asset", base_url))
      .header(("sec-ch-prefers-reduced-transparency", "legacy"))
      .prefers_reduced_transparency("\tReDuCe \t")
      .expect("Sec-CH-Prefers-Reduced-Transparency should be accepted")
      .emit()
      .expect("request should succeed");
  });
  let request = request_text(&request);
  ...
  assert_eq!(
    Some("reduce"),
    header_value(&request, "Sec-CH-Prefers-Reduced-Transparency")
  );
  assert_eq!(
    1,
    request
      ...
      .filter(|line| {
        line
          .to_ascii_lowercase()
          .starts_with("sec-ch-prefers-reduced-transparency:")
      })
      .count(),
    "typed Sec-CH-Prefers-Reduced-Transparency should replace an existing same-name field"
  );
}

#[cfg(feature = "async")]
#[test]
fn async_prefers_reduced_transparency_helper_matches_blocking_contract() {
  let request = capture_request(|base_url| {
    block_on(
      client()
        .get()
...
#[test]
fn prefers_reduced_transparency_helper_rejects_malformed_values_before_connecting() {
  let oversized = "a".repeat(64 * 1024 + 1);
  for value in [
    "",
    " ",
    ...
    let request = capture_optional_request(|base_url| {
      let error = client()
        ...
        .expect_err("invalid Sec-CH-Prefers-Reduced-Transparency input must be rejected");
      assert!(error.is_builder());
    });
    assert!(
      request.is_empty(),
      "invalid Sec-CH-Prefers-Reduced-Transparency input must not open a socket"
    );
  }
}
...
#[cfg(feature = "async")]
#[test]
fn async_prefers_reduced_transparency_helper_rejects_malformed_values_before_connecting() {
  let oversized = "a".repeat(64 * 1024 + 1);
  for value in [
    "",
    " ",
    ...
    let request = capture_optional_request(|base_url| {
      let error = block_on(async {
        let mut http_client = client();
        let request = http_client.get().url(format!("{}/asset", base_url));
        request.prefers_reduced_transparency(value)?.rasync().await
      })
      .expect_err("invalid Sec-CH-Prefers-Reduced-Transparency input must be rejected");
      assert!(error.is_builder());
    });
...
#[test]
fn raw_prefers_reduced_transparency_header_remains_available_as_escape_hatch() {
  let request = capture_request(|base_url| {
    client()
      .get()
      .url(format!("{}/asset", base_url))
      .header(("Sec-CH-Prefers-Reduced-Transparency", "legacy-token"))
      .emit()
      .expect("request should succeed");
  });
  let request = request_text(&request);
...
#[test]
fn sec_ch_viewport_width_helper_emits_one_canonical_request_client_hint() {
  let request = capture_request(|base_url| {
    client()
      .get()
      .url(format!("{}/asset", base_url))
      .header(("sec-ch-viewport-width", "2"))
  ...
  assert_eq!(
    Some("1440"),
    header_value(&request, "Sec-CH-Viewport-Width")
  );
  assert_eq!(
    1,
    request
      ...
      .filter(|line| line
        .to_ascii_lowercase()
        .starts_with("sec-ch-viewport-width:"))
      .count(),
    "typed Sec-CH-Viewport-Width should replace an existing same-name field"
  );
}

#[cfg(feature = "async")]
#[test]
fn async_sec_ch_viewport_width_helper_matches_blocking_contract() {
  let request = capture_request(|base_url| {
    block_on(
      client()
        .get()
...
#[test]
fn sec_ch_viewport_width_helper_rejects_malformed_values_before_connecting() {
  let oversized = "1".repeat(64 * 1024 + 1);
  for value in [
    "",
    "-1",
    ...
    let request = capture_optional_request(|base_url| {
      let error = client()
        .get()
        .url(format!("{}/asset", base_url))
        .sec_ch_viewport_width(value)
        .expect_err("invalid Sec-CH-Viewport-Width input must be rejected");
      assert!(error.is_builder());
    });
    assert!(
      request.is_empty(),
      "invalid Sec-CH-Viewport-Width input must not open a socket"
    );
  }
}
...
#[cfg(feature = "async")]
#[test]
fn async_sec_ch_viewport_width_helper_rejects_malformed_values_before_connecting() {
  let oversized = "1".repeat(64 * 1024 + 1);
  for value in [
    "",
    "-1",
    ...
    let request = capture_optional_request(|base_url| {
      let error = block_on(async {
        let mut http_client = client();
        let request = http_client.get().url(format!("{}/asset", base_url));
        request.sec_ch_viewport_width(value)?.rasync().await
      })
      .expect_err("invalid Sec-CH-Viewport-Width input must be rejected");
      assert!(error.is_builder());
    });
...
#[test]
fn raw_sec_ch_viewport_width_header_remains_available_as_escape_hatch() {
  let request = capture_request(|base_url| {
    client()
      .get()
      .url(format!("{}/asset", base_url))
      .header(("Sec-CH-Viewport-Width", "legacy-token"))
      .emit()
      .expect("request should succeed");
  });
  let request = request_text(&request);
