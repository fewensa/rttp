...
#[test]
fn http2_upgrade_raw_sec_ch_ua_mobile_preserves_value_outside_typed_grammar() {
  let listener = TcpListener::bind("127.0.0.1:0").expect("bind h2c upgrade peer");
  let addr = listener.local_addr().expect("h2c upgrade peer addr");
  ...
  let handle = thread::spawn(move || {
    let (mut stream, _) = listener.accept().expect("accept h2c upgrade client");
    let request = String::from_utf8(read_http1_request_head(&mut stream)).expect("request utf8");
    assert!(request.starts_with("GET /upgrade-raw-sec-ch-ua-mobile HTTP/1.1\r\n"));
    assert!(request.contains("\r\nConnection: Upgrade, HTTP2-Settings\r\n"));
    assert!(request.contains("\r\nUpgrade: h2c\r\n"));
    ...
    assert_eq!(3, request_headers.stream_id);
    assert_eq!(
      b"/upgrade-raw-sec-ch-ua-mobile",
      find_header_value(&request_headers.payload, b":path")
        .expect("request path")
        .value
        .as_slice()
    ...
    assert_eq!(
      b"true",
      find_header_value(&request_headers.payload, b"sec-ch-ua-mobile")
        .expect("sec-ch-ua-mobile")
        .value
        .as_slice()
    );
    ...
    write_frame(
      ...
      FLAG_END_STREAM,
      3,
      b"raw sec-ch-ua-mobile",
    );
  });

  let response = HttpClient::new()
    .get()
    .url(format!("http://{}/upgrade-raw-sec-ch-ua-mobile", addr))
    .header(("Sec-CH-UA-Mobile", "true"))
    .emit_http2_upgrade()
    .expect("h2c upgrade response");

  assert_eq!(200, response.code());
  assert_eq!("HTTP/2", response.version());
  assert_eq!("raw sec-ch-ua-mobile", response.body().string().unwrap());

  handle
    .join()
    .expect("h2c upgrade raw sec-ch-ua-mobile peer thread");
}
