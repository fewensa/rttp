...
#[test]
fn permissions_policy_report_only_metadata_parses_without_enforcing_or_reporting() {
  let value = r#"geolocation=(self "https://maps.example.test"), camera=()"#;
  let response = Response::new(
    RoUrl::with("https://example.test"),
    format!(
  ...
  assert_eq!(metadata.header_value(), value);
  assert_eq!(
    response.header_value("Permissions-Policy-Report-Only"),
    Some(&value.to_string())
  );
}

#[test]
fn permissions_policy_report_only_metadata_rejects_invalid_values_without_hiding_raw_headers() {
  for value in [
    "",
    "geolocation=src",
    "geolocation=('none')",
    ...
    assert!(
      response.permissions_policy_report_only().is_err(),
      "should reject {value:?}"
    );
    assert_eq!(
      response.header_value("Permissions-Policy-Report-Only"),
  ...
  let response = Response::new(
    ...
    concat!(
      "HTTP/1.1 200 OK\r\n",
      "Permissions-Policy-Report-Only: geolocation=(self)\r\n",
      "Permissions-Policy-Report-Only: geolocation=(self)\r\n",
      "Content-Length: 0\r\n\r\n"
    )
    .as_bytes()
  ...
  assert!(response.permissions_policy_report_only().is_err());
  assert_eq!(
...
fn permissions_policy_report_only_metadata_rejects_oversized_values_and_absent_headers() {
  ...
  let response = Response::new(
    RoUrl::with("https://example.test"),
    format!(
      "HTTP/1.1 200 OK\r\nPermissions-Policy-Report-Only: {oversized}\r\nContent-Length: 0\r\n\r\n"
    )
    .into_bytes(),
  )
  .expect("response should parse");
  ...
  let _: Option<PermissionsPolicyReportOnly> = absent
    .permissions_policy_report_only()
    .expect("header is absent");
}
