use rttp_client::response::{
  AcceptPatch, AcceptPatchParseError, MediaType, MediaTypeParameter, Response,
};
...
#[test]
fn response_accept_patch_uses_protocol_media_types_across_repeated_fields() {
...
#[test]
fn response_accept_patch_parse_failures_preserve_raw_headers() {
  for value in [
    "application/json,",
    ...
    assert_eq!(
      ...
      response.header_value("Accept-Patch")
    );
  }
}

#[test]
fn response_accept_patch_is_absent_without_a_header_and_exports_parse_error() {
  let response = Response::new(
    RoUrl::with("https://example.test"),
    b"HTTP/1.1 204 No Content\r\nContent-Length: 0\r\n\r\n".to_vec(),
  )
  .expect("response without Accept-Patch should parse");
  assert_eq!(
    None,
    response
      .accept_patch()
  ...
  let _: AcceptPatchParseError =
    AcceptPatch::parse("application/json,").expect_err("malformed metadata should fail");
}

#[test]
fn response_accept_patch_enforces_member_bound_without_losing_raw_header() {
  let value = std::iter::repeat_n("application/json", 257)
    .collect::<Vec<_>>()
    .join(", ");
  let response = response_with_values(&[&value]);
  assert!(response.accept_patch().is_err());
  assert_eq!(Some(&value), response.header_value("Accept-Patch"));
}
