...
use rttp_protocol::accept::{
  Accept, AcceptMediaRange, MAX_ACCEPT_VALUE_BYTES, MAX_CLIENT_ACCEPT_MEDIA_RANGES,
};
...
impl HttpClient {
  ...
  /// Set bounded `Device-Memory` request Client Hint metadata.
  ///
  /// The value must be one non-negative finite decimal GiB value with
  /// optional surrounding HTTP optional whitespace. This replaces any
  /// existing case-insensitive `Device-Memory` field and only declares
  /// request metadata; RTTP does not negotiate content, emit `Accept-CH`, or
  /// generate this header automatically.
  pub fn device_memory<S: AsRef<str>>(&mut self, value: S) -> error::Result<&mut Self> {
  ...
  /// Set bounded `Sec-CH-UA-Platform` request Client Hint metadata.
  ///
  ...
  /// `Sec-CH-UA-Platform` field and only declares request metadata; RTTP does
  /// not infer a platform, negotiate the UA brands family, emit `Accept-CH`,
  /// or generate this header automatically.
  pub fn sec_ch_ua_platform<S: AsRef<str>>(&mut self, value: S) -> error::Result<&mut Self> {
    let sec_ch_ua_platform = SecChUaPlatform::parse(value.as_ref())
      .map_err(|parse_error| error::builder_with_message(parse_error.to_string()))?;
    Ok(self.header(Header::new(
      "Sec-CH-UA-Platform",
  ...
  /// Set bounded `Sec-CH-UA-Arch` request Client Hint metadata.
  ///
  /// The value must be one Structured Fields string with optional surrounding
  /// HTTP optional whitespace. This replaces any existing case-insensitive
  /// `Sec-CH-UA-Arch` field and only declares request metadata; RTTP does not
  /// infer an architecture, negotiate the UA brands family, emit `Accept-CH`,
  /// or generate this header automatically.
  pub fn sec_ch_ua_arch<S: AsRef<str>>(&mut self, value: S) -> error::Result<&mut Self> {
  ...
  /// Set bounded `Sec-CH-UA` request Client Hint metadata.
  ///
  ...
  /// `Sec-CH-UA` field and only declares request metadata; RTTP does not
  /// infer browser identity, negotiate the UA brands family, emit `Accept-CH`,
  /// or apply browser policy.
  pub fn sec_ch_ua<S: AsRef<str>>(&mut self, value: S) -> error::Result<&mut Self> {
    let sec_ch_ua = SecChUa::parse(value.as_ref())
      .map_err(|parse_error| error::builder_with_message(parse_error.to_string()))?;
    Ok(self.header(Header::new("Sec-CH-UA", sec_ch_ua.header_value())))
  }
  ...
  /// Set bounded `Sec-CH-Prefers-Reduced-Data` request Client Hint metadata.
  ///
  /// The value must be the `no-preference` or `reduce` token, matched
  /// case-insensitively with optional surrounding HTTP optional whitespace.
  /// This replaces any existing case-insensitive
  /// `Sec-CH-Prefers-Reduced-Data` field and only declares request metadata;
  /// RTTP does not infer preferences, adapt content, negotiate `Accept-CH`,
  /// or generate this header automatically.
  pub fn prefers_reduced_data<S: AsRef<str>>(&mut self, value: S) -> error::Result<&mut Self> {
    let prefers_reduced_data = PrefersReducedData::parse(value.as_ref())
      .map_err(|parse_error| error::builder_with_message(parse_error.to_string()))?;
    Ok(self.header(Header::new(
      "Sec-CH-Prefers-Reduced-Data",
      prefers_reduced_data.header_value(),
    )))
  }
  ...
  /// Set bounded `Sec-CH-Viewport-Height` request Client Hint metadata.
  ///
  /// The value must be one non-negative decimal integer with optional
  /// surrounding HTTP optional whitespace. This replaces any existing
  /// case-insensitive `Sec-CH-Viewport-Height` field and only declares request
  /// metadata; RTTP does not negotiate content, emit `Accept-CH`, track
  /// viewport size, or generate this header automatically.
  pub fn sec_ch_viewport_height<S: AsRef<str>>(&mut self, value: S) -> error::Result<&mut Self> {
    let viewport_height = SecChViewportHeight::parse(value.as_ref())
      .map_err(|parse_error| error::builder_with_message(parse_error.to_string()))?;
    Ok(self.header(Header::new(
  ...
  /// Set bounded `RTT` request Client Hint metadata.
  ///
  /// The value must be one non-negative decimal millisecond integer with
  /// optional surrounding HTTP optional whitespace. This replaces any existing
  /// case-insensitive `RTT` field and only declares request metadata; RTTP
  /// does not negotiate content, emit `Accept-CH`, or generate this header
  /// automatically.
  pub fn rtt<S: AsRef<str>>(&mut self, value: S) -> error::Result<&mut Self> {
    let rtt = Rtt::parse(value.as_ref())
      .map_err(|parse_error| error::builder_with_message(parse_error.to_string()))?;
    Ok(self.header(Header::new("RTT", rtt.header_value())))
  }
  ...
  fn accept_member(&mut self, media_range: &str, qvalue: Option<&str>) -> error::Result<&mut Self> {
    let member = AcceptMediaRange::request_builder_member(media_range, qvalue)
      .map_err(|error| error::builder_with_message(error.to_string()))?;
    ...
    if let Some(header) = existing {
      let existing = Accept::parse_request_builder_values_with_limit(
        [header.value().as_str()],
        MAX_CLIENT_ACCEPT_MEDIA_RANGES,
      )
      .map_err(|error| error::builder_with_message(error.to_string()))?;
      if existing.len() >= MAX_CLIENT_ACCEPT_MEDIA_RANGES {
