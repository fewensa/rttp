...
head returns the typed bad-response error. Each informational head is limited
...
| Informational responses and Early Hints | `Response::informational_responses` exposes up to 16 skipped HTTP/1.1 `1xx` heads, including `103 Early Hints`, with preserved raw headers | A seventeenth skippable head returns a typed bad-response error; `101 Switching Protocols` remains terminal for upgrade handoff; no automatic preload execution, cache policy, redirect/retry/replay, route generation, streaming early-write API, TLS/ALPN behavior, or status-policy behavior |
