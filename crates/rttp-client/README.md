...
| Set-Cookie | `Response::set_cookies()` parses bounded protocol `Set-Cookie` response metadata, preserves multiple field lines and raw headers, and redacts cookie values from typed debug and errors; `Response::cookies()`/`cookie()` remain a legacy compatibility view | No cookie jar, persistence, domain/path matching, expiry enforcement, SameSite or partitioning policy, or automatic request `Cookie` emission |
