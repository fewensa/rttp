...
through the shared protocol `AcceptPatch` type. Repeated fields are combined
...
re-exports `AcceptPatch`, `AcceptPatchParseError`, `MediaType`, and
...
without discarding the raw `Accept-Patch` fields available through
`Response::header_value()` and `Response::header_values()`.

This is response metadata only. `rttp_client` does not route `PATCH` requests,
...
The parser is bounded and validation-oriented. At most 16 skippable
informational heads may be retained in one response history, including heads
accumulated across `Expect: 100-continue` handling; a seventeenth skippable
head returns the typed bad-response error as soon as that combined count is
exceeded, including post-`100 Continue` heads read after the request body is
sent. Each informational head is limited
to the normal response-head bound, must use an HTTP/1.1 `1xx` status line,
must contain valid header field names and values, and must not declare body
framing with `Content-Length` or `Transfer-Encoding`. Malformed, oversized,
or ambiguously framed informational heads return an error before the final
response head is consumed; raw header fields from valid skipped heads are
preserved on the informational metadata.
...
`Sec-CH-UA-Platform` request field through the shared `SecChUaPlatform` type,
...
`HttpClient::sec_ch_ua_model(value)` validates and emits one bounded
`Sec-CH-UA-Model` request field through the shared `SecChUaModel` type,
replacing any existing case-insensitive field. The value must be one RFC 8941
Structured Fields string; optional surrounding SP or HTAB is trimmed and
canonical quoted-string escaping is emitted. Empty, non-string, comma-list,
parameterized, duplicate, non-ASCII, forbidden-control, invalid escape, and
oversized values are rejected before connecting. This helper declares metadata
only; it does not infer device identity, negotiate the UA brands family, emit
`Accept-CH`, generate Client Hints automatically, retry, or apply browser
policy. Raw values remain available through
...
`HttpClient::sec_ch_ua_bitness(value)` validates and emits one bounded
`Sec-CH-UA-Bitness` request field through the shared `SecChUaBitness` type,
replacing any existing case-insensitive field. The value must be one
Structured Fields string; optional surrounding SP or HTAB is trimmed and
canonical quoted-string escaping is emitted. Empty, non-string, comma-list,
parameterized, duplicate, non-ASCII, forbidden-control, invalid escape, and
...
request field through the shared `SecChUa` type, replacing any existing
case-insensitive field. The value is an ordered RFC 8941 list of string brands;
each member must carry exactly one string `v` parameter. The parser bounds each
field value and aggregate input to 64 KiB, accepts at most 256 entries, preserves
order, and canonicalizes valid Structured Fields quoting. Empty or malformed
lists, non-ASCII or forbidden-control input, missing/extra/duplicate parameters,
duplicate fields, excessive entries, and oversized values are rejected before
connecting. This helper declares metadata only; it does not infer browser
identity, negotiate the UA brands family, emit `Accept-CH`, generate Client
Hints automatically, retry, or apply browser policy. Raw values remain
available through `header(("Sec-CH-UA", "..."))`.
...
`Sec-CH-Prefers-Reduced-Motion` request field through the shared
`PrefersReducedMotion` type, replacing any existing case-insensitive field.
`PrefersReducedMotion::parse()` accepts `no-preference` and `reduce`
...
duplicate, control-byte, and oversized values are rejected before connecting.
This helper declares metadata only; it does not infer a preference, adapt
content, negotiate `Accept-CH`, generate Client Hints automatically, retry, or
...
`Sec-CH-Prefers-Contrast` request field through the shared `PrefersContrast`
type, replacing any existing case-insensitive field. `PrefersContrast::parse()`
accepts `no-preference`, `more`, `less`, and `custom` case-insensitively with
optional surrounding SP or HTAB, while `header_value()` emits lowercase
canonical text. Empty, unknown, comma-list, duplicate, control-byte, and
oversized values are rejected before connecting. This helper declares metadata
only; it does not infer a preference, adapt content, negotiate `Accept-CH`,
generate Client Hints automatically, retry, or apply browser policy. Raw values
remain available through `header(("Sec-CH-Prefers-Contrast", "..."))`.
...
`HttpClient::sec_ch_viewport_height(value)` validates and emits one bounded
`Sec-CH-Viewport-Height` request field through the shared `SecChViewportHeight`
type, replacing any existing same-name field. `SecChViewportHeight::value()`
exposes the non-negative unsigned integer and
`SecChViewportHeight::header_value()` returns canonical decimal text. Invalid,
signed, fractional, overflowing, control-byte, duplicate, and oversized values
are rejected before connecting. This helper declares metadata only; it does not
negotiate content, emit `Accept-CH`, track viewport size, or adapt
...
| Informational responses and Early Hints | `Response::informational_responses` exposes up to 16 skipped HTTP/1.1 `1xx` heads, including `103 Early Hints`, with preserved raw headers | A seventeenth skippable head, including a post-`100 Continue` head that exceeds the combined history, returns a typed bad-response error immediately; `101 Switching Protocols` remains terminal for upgrade handoff; no automatic preload execution, cache policy, redirect/retry/replay, route generation, streaming early-write API, TLS/ALPN behavior, or status-policy behavior |
