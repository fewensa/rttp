...
| Referrer-Policy | `Response::referrer_policy` parses one or more `Referrer-Policy` response fields into ordered bounded metadata; recognized tokens parse case-insensitively with ordering and repeats preserved, valid unknown tokens are ignored for forward compatibility, every member is validated, each field value is limited to 64 KiB, the cumulative member count across recognized and unknown tokens and across all fields is limited to 256, and forbidden ASCII control bytes such as CR, LF, NUL, DEL, and inner HTAB are rejected in every member while raw headers remain available | No browser policy enforcement, Referer behavior changes, redirect policy, retry/replay, or status-policy behavior |
