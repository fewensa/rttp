...
| Sec-CH-UA-Form-Factors | `SecChUaFormFactors::parse`, `HttpClient::sec_ch_ua_form_factors`, `Request::sec_ch_ua_form_factors`, and `HttpRequest::sec_ch_ua_form_factors` validate, emit, or parse a bounded RFC 8941 list of ordered structured strings while preserving raw headers on errors | No device-class inference, UA brands family, `Accept-CH` negotiation, retries, or browser policy |
