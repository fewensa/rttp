...
| Referrer-Policy | `Response::referrer_policy` parses bounded response `Referrer-Policy` fields into ordered recognized token metadata, ignoring valid unknown tokens while rejecting malformed tokens, forbidden ASCII controls outside delimiter HTAB whitespace, and more than 256 total tokens across fields | No browser referrer policy, redirect behavior, cache policy, retry, replay, or header generation |
