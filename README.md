...
Codeon full-flow verification status: COMPLETE.
...
`example.test` is a reserved documentation domain used here as a placeholder;
replace it with your own endpoint when running this example.
...
`Response::accept_patch()` parses one or more `Accept-Patch` response fields
through the shared protocol `AcceptPatch` type. Repeated fields are flattened
...
and the `rttp` facade re-export `AcceptPatch` and
`AcceptPatchParseError` (as well as the media-type inspection types).
...
This is metadata-only behavior. RTTP does not route `PATCH` requests, decode
...
The skipped head parser is bounded and validation-oriented. At most 16
skippable informational heads are retained in one response history, including
heads accumulated across `Expect: 100-continue` handling; a seventeenth
skippable head returns the typed bad-response error. Each informational head
is limited to the normal response-head bound, must be an HTTP/1.1 `1xx`
...
received `100 Continue` alongside other informational responses, within the
same maximum-16 retained history; a seventeenth skippable response is rejected.
...
On the server, `Request::from()` and `HttpRequest::from()` parse all
case-insensitive `From` request fields through the same protocol type. They
return `Ok(None)` when absent and expose canonical `header_value()`,
`address()`, `local_part()`, `domain()`, and `display_name()` accessors.
Duplicate, malformed, non-ASCII, control-byte, and oversized fields return
`HttpFromParseError`; raw values remain available through `Request::header()`
or `HttpRequest::header()` after typed parsing fails.
...
On the server, `Request::referer()` and `HttpRequest::referer()` parse all
case-insensitive `Referer` request fields through the same protocol type as
`HttpReferer`. They return `Ok(None)` when absent and expose the OWS-trimmed
URI reference through `header_value()`. Duplicate, fragment, malformed
percent-encoding, control-byte, empty, and oversized fields return
`HttpRefererParseError`; raw values remain available through
...
|------|-----------------|--------|
| HTTP/1.1 response parsing | `Content-Length`, chunked transfer coding, chunk extensions, up to 16 retained informational responses, `Response::is_informational`, `is_redirection`, `is_error`, bodyless `204`/`304`, duplicate `Set-Cookie`, and framing ambiguity rejection | A seventeenth skippable informational response is rejected; not a complete RFC conformance suite |
...
| HTTP message signatures | Client `signature`, `signature_input`, and `accept_signature` emit bounded RFC 9421 request metadata; server `Request`/`HttpRequest` parse `Signature`, `Signature-Input`, and `Accept-Signature`; `HttpResponse::with_accept_signature`/`accept_signature` and client `Response::accept_signature` declare or parse response metadata while preserving raw headers on parse failures; repeated `Accept-Signature` fields combine in wire order and registered request parameters retain valueless `created`/`expires` plus typed string metadata | No signing, verification, key lookup, covered-component canonicalization, cryptographic policy, or signature negotiation |
...
| From | Client `from` emits one bounded canonical `From` request field through the shared protocol type, replacing existing case-insensitive fields; the value is one bare mailbox or name-address, and raw `header(("From", value))` remains available as a fallback. Server `Request::from` and `HttpRequest::from` parse the received singleton through the same type, reject duplicate, malformed, non-ASCII, control-byte, and oversized fields, and preserve raw headers on errors | No identity or ownership verification, privacy or authorization policy, deliverability assessment, DNS or SMTP work, or email delivery |
| Referer | Client `referer` emits one bounded canonical `Referer` request field through the shared protocol type, replacing existing case-insensitive fields; absolute, relative, and scheme-relative URI references are accepted, and raw `header(("Referer", value))` remains available as a fallback. Server `Request::referer` and `HttpRequest::referer` parse the received singleton through the same type as `HttpReferer`, reject duplicate, fragment, malformed percent-encoding, control-byte, empty, and oversized fields, and preserve raw headers on errors | No `Referrer-Policy` enforcement, trust decisions, CSRF protection, redaction, URL canonicalization, or redirect behavior |
...
| Sec-CH-UA-Model request Client Hint | `HttpClient::sec_ch_ua_model` emits bounded singleton `Sec-CH-UA-Model` metadata through `SecChUaModel`; server `Request::sec_ch_ua_model()` and `HttpRequest::sec_ch_ua_model()` parse one RFC 8941 Structured Fields string and preserve raw values on errors | No device-identity inference, UA brands family, `Accept-CH` negotiation, retries, or browser policy |
...
| Sec-CH-Viewport-Height request Client Hint | `HttpClient::sec_ch_viewport_height` emits bounded singleton `Sec-CH-Viewport-Height` request metadata through `SecChViewportHeight`; server `Request::sec_ch_viewport_height()` and `HttpRequest::sec_ch_viewport_height()` parse received fields as `HttpSecChViewportHeight` while preserving raw values on errors | No content negotiation, `Accept-CH` emission, automatic Client Hints generation, retry, viewport tracking, or content adaptation |
| Sec-CH-Viewport-Width request Client Hint | `HttpClient::sec_ch_viewport_width` emits bounded singleton `Sec-CH-Viewport-Width` request metadata through `SecChViewportWidth`; server `Request::sec_ch_viewport_width()` and `HttpRequest::sec_ch_viewport_width()` parse received fields as `HttpSecChViewportWidth` while preserving raw values on errors | No content negotiation, `Accept-CH` emission, automatic Client Hints generation, retry, viewport tracking, or content adaptation |
| RTT request Client Hint | `HttpClient::rtt` emits bounded singleton `RTT` request metadata through `Rtt`; server `Request::rtt()` and `HttpRequest::rtt()` parse received fields as `HttpRtt` while preserving raw values on errors | No content negotiation, `Accept-CH` emission, automatic Client Hints generation, retry, network-delay measurement, or content adaptation |
...
| Informational responses and Early Hints | `Response::informational_responses` exposes up to 16 skipped HTTP/1.1 `1xx` heads, including `103 Early Hints`, with preserved raw headers; server `HttpResponse::early_hints`/`early_hints_with_headers` construct validated bodyless `103` metadata | A seventeenth skippable head returns a typed bad-response error; `101 Switching Protocols` remains terminal for upgrade handoff; no automatic preload execution, cache policy, redirect/retry/replay, route generation, streaming early-write API, TLS/ALPN behavior, or status-policy behavior |
...
| Sec-CH-UA-Model | `SecChUaModel::parse`, `HttpClient::sec_ch_ua_model`, `Request::sec_ch_ua_model`, and `HttpRequest::sec_ch_ua_model` validate, emit, or parse bounded singleton RFC 8941 Structured Fields string request Client Hint metadata while preserving raw headers on errors | No device-identity inference, UA brands family, `Accept-CH` negotiation, retries, or browser policy |
...
| Sec-CH-Viewport-Height | `SecChViewportHeight::parse`, `HttpClient::sec_ch_viewport_height`, `Request::sec_ch_viewport_height`, and `HttpRequest::sec_ch_viewport_height` validate, emit, or parse bounded singleton request Client Hint metadata while preserving raw headers on errors | No content negotiation, `Accept-CH` emission, automatic Client Hints generation, retry, viewport tracking, or content adaptation |
| Sec-CH-Viewport-Width | `SecChViewportWidth::parse`, `HttpClient::sec_ch_viewport_width`, `Request::sec_ch_viewport_width`, and `HttpRequest::sec_ch_viewport_width` validate, emit, or parse bounded singleton request Client Hint metadata while preserving raw headers on errors | No content negotiation, `Accept-CH` emission, automatic Client Hints generation, retry, viewport tracking, or content adaptation |
| RTT | `Rtt::parse`, `HttpClient::rtt`, `Request::rtt`, and `HttpRequest::rtt` validate, emit, or parse bounded singleton request Client Hint metadata while preserving raw headers on errors | No content negotiation, `Accept-CH` emission, automatic Client Hints generation, retry, network-delay measurement, or content adaptation |
...
`HttpResponse::with_accept_patch(media_types)` joins an ordered list of
caller-supplied media-type values, validates and serializes it through the
shared protocol `HttpAcceptPatch` type, and replaces existing
`Accept-Patch` fields with one validated field. `HttpResponse::accept_patch()`
parses attached fields in wire order into the same type. The server facade
exposes `HttpAcceptPatch`, `HttpAcceptPatchParseError`, `HttpMediaType`, and
...
`HttpResponse::header("Expires", ...)`, `HttpResponse::header("Last-Modified", ...)`,
and `HttpResponse::header("Retry-After", ...)` values remain preserved exactly
as response headers. These helpers do not calculate freshness, correct clock
skew, validate cache state against wall-clock time, store responses, match
stored responses, revalidate responses, enforce shared-cache policy, attach
behavior to status codes, sleep, retry, replay requests, apply backoff,
integrate with a scheduler,
...
`HttpClient::downlink(value)` validates and emits one singleton `Downlink`
request Client Hint through the shared `Downlink` type. `Request::downlink()`
and `HttpRequest::downlink()` parse received fields into `HttpDownlink`,
exposing the non-negative finite decimal Mbps value with `mbps()` and the
trimmed decimal text with `header_value()`. Invalid, duplicate, non-finite,
negative, control-byte, and oversized values return parser errors while raw
headers remain available; `header(("Downlink", "..."))` remains an escape hatch
for unvalidated values.
...
`HttpClient::prefers_color_scheme(value)` validates and emits one singleton
`Sec-CH-Prefers-Color-Scheme` request Client Hint through the shared
`PrefersColorScheme` type. `Request::prefers_color_scheme()` and
`HttpRequest::prefers_color_scheme()` parse received fields into
`HttpPrefersColorScheme`, accepting `light` and `dark` case-insensitively after
trimming optional SP or HTAB and exposing lowercase canonical text through
`header_value()`. Empty, unknown, comma-list, duplicate, control-byte, and
oversized values return parser errors while raw headers remain available;
`header(("Sec-CH-Prefers-Color-Scheme", "..."))` remains an escape hatch.
...
`HttpClient::sec_ch_ua_platform(value)` validates and emits one singleton
`Sec-CH-UA-Platform` request Client Hint through the shared `SecChUaPlatform`
type. `Request::sec_ch_ua_platform()` and `HttpRequest::sec_ch_ua_platform()`
parse received fields into `HttpSecChUaPlatform`, accepting one Structured
Fields string after trimming optional SP or HTAB and exposing canonical
quoted-string text through `header_value()`. Empty, non-string, comma-list,
...
### Bounded Sec-CH-UA-Model request Client Hint metadata

`HttpClient::sec_ch_ua_model(value)` validates and emits one singleton
`Sec-CH-UA-Model` request Client Hint through the shared `SecChUaModel` type.
`Request::sec_ch_ua_model()` and `HttpRequest::sec_ch_ua_model()` parse received
fields into `HttpSecChUaModel`, accepting one RFC 8941 Structured Fields string
and preserving raw headers on parse errors. Empty, non-string, comma-list,
parameterized, duplicate, non-ASCII, forbidden-control, invalid-escape, and
oversized values are rejected. These helpers are metadata-only: RTTP does not
infer device identity, negotiate the UA brands family, emit `Accept-CH`, retry,
or apply browser policy.

### Bounded Sec-CH-UA-Arch request Client Hint metadata

`HttpClient::sec_ch_ua_arch(value)` validates and emits one singleton
`Sec-CH-UA-Arch` request Client Hint through the shared `SecChUaArch` type.
`Request::sec_ch_ua_arch()` and `HttpRequest::sec_ch_ua_arch()` parse received
fields into `HttpSecChUaArch`, accepting one Structured Fields string after
trimming optional SP or HTAB and exposing canonical quoted-string text through
...
parser errors while raw headers remain available;
`header(("Sec-CH-UA-Arch", "..."))` remains an escape hatch.

These helpers are metadata-only. RTTP does not infer an architecture, negotiate
the UA brands family, emit `Accept-CH`, generate Client Hints automatically,
retry, replay, redirect, or apply browser policy from `Sec-CH-UA-Arch`.

### Bounded Sec-CH-UA-Bitness request Client Hint metadata

`HttpClient::sec_ch_ua_bitness(value)` validates and emits one singleton
`Sec-CH-UA-Bitness` request Client Hint through the shared `SecChUaBitness`
type. `Request::sec_ch_ua_bitness()` and `HttpRequest::sec_ch_ua_bitness()`
parse received fields into `HttpSecChUaBitness`, accepting one Structured
Fields string after trimming optional SP or HTAB and exposing canonical
quoted-string text through `header_value()`. Empty, non-string, comma-list,
parameterized, duplicate, non-ASCII, forbidden-control, invalid-escape, and
oversized values return parser errors while raw headers remain available;
`header(("Sec-CH-UA-Bitness", "..."))` remains an escape hatch.

These helpers are metadata-only. RTTP does not infer an architecture, negotiate
the UA brands family, emit `Accept-CH`, generate Client Hints automatically,
retry, replay, redirect, or apply browser policy from `Sec-CH-UA-Bitness`.

### Bounded Sec-CH-UA-Platform-Version request Client Hint metadata

`HttpClient::sec_ch_ua_platform_version(value)` validates and emits one
singleton `Sec-CH-UA-Platform-Version` request Client Hint through the shared
`SecChUaPlatformVersion` type. `Request::sec_ch_ua_platform_version()` and
`HttpRequest::sec_ch_ua_platform_version()` parse received fields into
`HttpSecChUaPlatformVersion`, accepting one Structured Fields string after
trimming optional SP or HTAB and exposing canonical quoted-string text through
`header_value()`. Empty, non-string, comma-list, parameterized, duplicate,
non-ASCII, forbidden-control, invalid-escape, and oversized values return
parser errors while raw headers remain available;
`header(("Sec-CH-UA-Platform-Version", "..."))` remains an escape hatch.

These helpers are metadata-only. RTTP does not infer a platform version or
...
`HttpClient::prefers_reduced_motion(value)` validates and emits one singleton
`Sec-CH-Prefers-Reduced-Motion` request Client Hint through the shared
`PrefersReducedMotion` type. `Request::prefers_reduced_motion()` and
...
remain available; `header(("Sec-CH-Prefers-Reduced-Motion", "..."))` remains
an escape hatch.

These helpers are metadata-only. RTTP does not infer a user preference, adapt
content, negotiate `Accept-CH`, generate Client Hints automatically, retry,
replay, redirect, or apply browser policy from `Sec-CH-Prefers-Reduced-Motion`.
...
singleton `Sec-CH-Prefers-Reduced-Transparency` request Client Hint through
the shared `PrefersReducedTransparency` type. `Request::prefers_reduced_transparency()`
and `HttpRequest::prefers_reduced_transparency()` parse received fields into
...
`header(("Sec-CH-Prefers-Reduced-Transparency", "..."))` remains an escape
hatch.
...
`HttpClient::prefers_contrast(value)` validates and emits one singleton
`Sec-CH-Prefers-Contrast` request Client Hint, bounded to 64 KiB, through the shared
`PrefersContrast` type. `Request::prefers_contrast()` and
`HttpRequest::prefers_contrast()` parse received fields into
`HttpPrefersContrast`, accepting `no-preference`, `more`, `less`, and `custom`
case-insensitively after trimming optional SP or HTAB and exposing lowercase
canonical text through `header_value()`. Empty, unknown, comma-list, duplicate,
control-byte, and oversized values return parser errors while raw headers
remain available; `header(("Sec-CH-Prefers-Contrast", "..."))` remains an
escape hatch.

These helpers are metadata-only. RTTP does not infer a user preference, adapt
content, negotiate `Accept-CH`, generate Client Hints automatically, retry,
replay, redirect, or apply browser policy from `Sec-CH-Prefers-Contrast`.
...
Client Hint through the shared `Width` type. `Request::width()` and
`HttpRequest::width()` parse received fields into `HttpWidth`, exposing the
non-negative integer with `value()` and canonical text with `header_value()`.
Invalid, duplicate, overflowing, control-byte, and oversized values return
parser errors while raw headers remain available.

These helpers are metadata-only. RTTP does not negotiate content, emit
`Accept-CH`, generate Client Hints automatically, retry, replay, redirect, or
adapt representations from `Width`.

### Bounded Viewport-Width request Client Hint metadata

`HttpClient::viewport_width(value)` validates and emits one singleton
`Viewport-Width` request Client Hint through the shared `ViewportWidth` type.
`Request::viewport_width()` and `HttpRequest::viewport_width()` parse received
fields into `HttpViewportWidth`, exposing the non-negative integer with
`value()` and canonical text with `header_value()`. Invalid, duplicate,
overflowing, control-byte, and oversized values return parser errors while raw
headers remain available.
...
### Bounded Sec-CH-Viewport-Height request Client Hint metadata

`HttpClient::sec_ch_viewport_height(value)` validates and emits one singleton
`Sec-CH-Viewport-Height` request Client Hint through the shared
`SecChViewportHeight` type. `Request::sec_ch_viewport_height()` and
`HttpRequest::sec_ch_viewport_height()` parse received fields into
`HttpSecChViewportHeight`, exposing the non-negative integer with `value()` and
canonical text with `header_value()`. Invalid, duplicate, overflowing,
control-byte, and oversized values return parser errors while raw headers
remain available.

These helpers are metadata-only. RTTP does not negotiate content, emit
`Accept-CH`, generate Client Hints automatically, retry, replay, redirect,
track viewport size, or adapt representations from `Sec-CH-Viewport-Height`.

### Bounded Sec-CH-Viewport-Width request Client Hint metadata

`HttpClient::sec_ch_viewport_width(value)` validates and emits one singleton
`Sec-CH-Viewport-Width` request Client Hint through the shared
`SecChViewportWidth` type. `Request::sec_ch_viewport_width()` and
`HttpRequest::sec_ch_viewport_width()` parse received fields into
`HttpSecChViewportWidth`, exposing the non-negative integer with `value()` and
canonical text with `header_value()`. Invalid, duplicate, overflowing,
control-byte, and oversized values return parser errors while raw headers
remain available.
...
These helpers are metadata-only. RTTP does not negotiate content, emit
`Accept-CH`, generate Client Hints automatically, retry, replay, redirect,
track viewport size, or adapt representations from `Sec-CH-Viewport-Width`.

### Bounded RTT request Client Hint metadata

`HttpClient::rtt(value)` validates and emits one singleton `RTT` request
Client Hint through the shared `Rtt` type. `Request::rtt()` and
`HttpRequest::rtt()` parse received fields into `HttpRtt`, exposing the
non-negative millisecond integer with `value()` and canonical text with
`header_value()`. Invalid, duplicate, overflowing, control-byte, and oversized
values return parser errors while raw headers remain available.

These helpers are metadata-only. RTTP does not negotiate content, emit
`Accept-CH`, generate Client Hints automatically, retry, replay, redirect,
measure network delay, or adapt representations from `RTT`.
...
| From | `HttpFrom`, `Request::from`, and `HttpRequest::from` parse bounded singleton `From` request metadata through the shared protocol type, exposing bare mailbox or name-address accessors, rejecting duplicate, malformed, non-ASCII, control-byte, and oversized fields, and preserving raw values on errors | No identity or ownership verification, privacy or authorization policy, deliverability assessment, DNS or SMTP work, or email delivery |
| Referer | `HttpReferer`, `Request::referer`, and `HttpRequest::referer` parse bounded singleton `Referer` request metadata through the shared protocol type, accepting absolute, relative, and scheme-relative URI references, rejecting duplicate, fragment, malformed percent-encoding, control-byte, empty, and oversized fields, and preserving raw values on errors | No `Referrer-Policy` enforcement, trust decisions, CSRF protection, redaction, URL canonicalization, or redirect behavior |
