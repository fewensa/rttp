...
| Referrer-Policy | `Response::referrer_policy` parses bounded response `Referrer-Policy` fields into ordered recognized policy metadata, rejecting forbidden control bytes and malformed token syntax even inside unknown members, counting recognized and unknown tokens cumulatively across all fields, and preserving recognized order, repeats, and case-insensitive matching while ignoring valid unknown tokens | No browser referrer policy, referrer stripping, redirect behavior, or last-token selection |
...
| Referrer-Policy | `HttpResponse::with_referrer_policy` validates and replaces attached `Referrer-Policy` fields with one bounded non-empty value under 64 KiB with no control bytes except HTAB, and `HttpResponse::referrer_policy` parses a single attached raw field into `HttpReferrerPolicy` metadata while rejecting duplicate fields | No browser referrer policy, referrer stripping, redirect behavior, or policy enforcement |
