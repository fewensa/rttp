...
| ECT request Client Hint | `HttpClient::ect` emits bounded singleton `ECT` request metadata through `Ect`; server `Request::ect()` and `HttpRequest::ect()` parse standard `slow-2g`, `2g`, `3g`, or `4g` tokens case-insensitively as `HttpEct`, canonicalizing output to lowercase while preserving raw values on errors | No network-quality inference, content negotiation, `Accept-CH` emission, automatic Client Hints generation, retry, or Client Hints policy |
...
`HttpRequest::ect()` parse received fields into `HttpEct`, accepting the
standard `slow-2g`, `2g`, `3g`, and `4g` tokens with ASCII case-insensitive
RFC 5234 matching and exposing canonical lowercase text through
`header_value()`. Invalid, unknown, list, parameterized, duplicate,
control-byte, and oversized values return parser errors while raw headers
remain available.

These helpers are metadata-only. RTTP does not infer network quality,
negotiate content, emit `Accept-CH`, generate Client Hints automatically,
retry, replay, redirect, or apply Client Hints policy from `ECT`.
