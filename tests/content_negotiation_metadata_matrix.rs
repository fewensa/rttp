...
#[test]
fn sec_ch_ua_arch_parses_valid_duplicate_and_malformed_http11_headers() {
  let (addr, observed_rx, handle) = spawn_observed_facade_server(
    |request| {
      (
        request
  ...
  assert_eq!(
    ...
    observed_rx
      .recv_timeout(TIMEOUT)
      .expect("observe valid Sec-CH-UA-Arch")
  );
  handle.join().expect("valid arch server thread");
  ...
  assert!(observed_rx
    .recv_timeout(TIMEOUT)
    .expect("observe duplicate Sec-CH-UA-Arch")
    .is_err());
  handle.join().expect("duplicate arch server thread");
  ...
  assert!(observed_rx
    .recv_timeout(TIMEOUT)
    .expect("observe malformed Sec-CH-UA-Arch")
    .is_err());
  ...
  let observed = observed_rx
    .recv_timeout(TIMEOUT)
    .expect("observe non-ASCII Sec-CH-UA-Arch");
  assert!(observed.0.is_err());
  assert_eq!(Some("\"\u{0080}\"".to_owned()), observed.1);
  handle.join().expect("non-ASCII arch server thread");
