...
#[test]
fn sync_client_and_server_exchange_canonical_prefers_reduced_transparency_metadata() {
  let server = rttp_server::server::HttpServer::bind("127.0.0.1:0")
    .expect("bind Prefers-Reduced-Transparency server");
  let addr = server
    .local_addr()
    .expect("Prefers-Reduced-Transparency server addr");
  let (observed_tx, observed_rx) = mpsc::channel();
  let handle = thread::spawn(move || {
    server
      .accept_one(|request| {
        let observed = (
          ...
          request
            ...
            .map(|transparency| {
              transparency.map(|transparency| transparency.header_value().to_string())
            })
            .map_err(|error| error.to_string()),
        );
        observed_tx
          .send(observed)
          .expect("send observed Prefers-Reduced-Transparency metadata");
        HttpResponse::ok("OK")
      })
      .expect("serve Prefers-Reduced-Transparency metadata request");
  });
  ...
  assert_eq!(
    ...
    observed_rx
      .recv()
      .expect("server should observe Prefers-Reduced-Transparency metadata")
  );
  handle
    .join()
    .expect("Prefers-Reduced-Transparency server thread");
}

#[test]
fn facade_server_rejects_malformed_prefers_reduced_transparency_without_losing_raw_headers() {
  let server = rttp_server::server::HttpServer::bind("127.0.0.1:0")
    .expect("bind malformed Prefers-Reduced-Transparency server");
  let addr = server
    .local_addr()
    .expect("malformed Prefers-Reduced-Transparency server addr");
  let (observed_tx, observed_rx) = mpsc::channel();
  let handle = thread::spawn(move || {
    server
      .accept_one(|request| {
        observed_tx
          ...
          .expect("send observed malformed Prefers-Reduced-Transparency");
        HttpResponse::ok("OK")
      })
      .expect("serve malformed Prefers-Reduced-Transparency request");
  });

  let mut stream =
    TcpStream::connect(addr).expect("connect malformed Prefers-Reduced-Transparency");
  stream
    .write_all(
      b"GET /asset HTTP/1.1\r\nHost: example.test\r\nSec-CH-Prefers-Reduced-Transparency: auto\r\nConnection: close\r\n\r\n",
    )
    .expect("write malformed Prefers-Reduced-Transparency request");

  assert_eq!(
    (Some("auto".to_string()), true),
    observed_rx
      .recv_timeout(Duration::from_secs(1))
      .expect("server should observe malformed Prefers-Reduced-Transparency")
  );
  handle
    .join()
    .expect("malformed Prefers-Reduced-Transparency server thread");
}

#[test]
fn facade_server_rejects_duplicate_prefers_reduced_transparency_without_losing_raw_headers() {
  let server = rttp_server::server::HttpServer::bind("127.0.0.1:0")
    .expect("bind duplicate Prefers-Reduced-Transparency server");
  let addr = server
    .local_addr()
    .expect("duplicate Prefers-Reduced-Transparency server addr");
  let (observed_tx, observed_rx) = mpsc::channel();
  let handle = thread::spawn(move || {
    server
      .accept_one(|request| {
        observed_tx
          ...
          .expect("send observed duplicate Prefers-Reduced-Transparency");
        HttpResponse::ok("OK")
      })
      .expect("serve duplicate Prefers-Reduced-Transparency request");
  });

  let mut stream =
    TcpStream::connect(addr).expect("connect duplicate Prefers-Reduced-Transparency");
  stream
    .write_all(
      b"GET /asset HTTP/1.1\r\nHost: example.test\r\nSec-CH-Prefers-Reduced-Transparency: no-preference\r\nsec-ch-prefers-reduced-transparency: reduce\r\nConnection: close\r\n\r\n",
    )
    .expect("write duplicate Prefers-Reduced-Transparency request");

  assert_eq!(
    (Some("no-preference".to_string()), true),
    observed_rx
      .recv_timeout(Duration::from_secs(1))
      .expect("server should observe duplicate Prefers-Reduced-Transparency")
  );
  handle
    .join()
    .expect("duplicate Prefers-Reduced-Transparency server thread");
}

#[test]
fn facade_server_rejects_non_ascii_prefers_reduced_transparency_without_losing_raw_headers() {
  let server = rttp_server::server::HttpServer::bind("127.0.0.1:0")
    .expect("bind non-ASCII Prefers-Reduced-Transparency server");
  let addr = server
    .local_addr()
    .expect("non-ASCII Prefers-Reduced-Transparency server addr");
  let (observed_tx, observed_rx) = mpsc::channel();
  let handle = thread::spawn(move || {
    server
      .accept_one(|request| {
        observed_tx
          ...
          .expect("send observed non-ASCII Prefers-Reduced-Transparency");
        HttpResponse::ok("OK")
      })
      .expect("serve non-ASCII Prefers-Reduced-Transparency request");
  });

  let mut stream =
    TcpStream::connect(addr).expect("connect non-ASCII Prefers-Reduced-Transparency");
  stream
    .write_all(
      b"GET /asset HTTP/1.1\r\nHost: example.test\r\nSec-CH-Prefers-Reduced-Transparency: \x80\r\nConnection: close\r\n\r\n",
    )
    .expect("write non-ASCII Prefers-Reduced-Transparency request");

  assert_eq!(
    (Some("\u{0080}".to_string()), true),
    observed_rx
      .recv_timeout(Duration::from_secs(1))
      .expect("server should observe non-ASCII Prefers-Reduced-Transparency")
  );
  handle
    .join()
    .expect("non-ASCII Prefers-Reduced-Transparency server thread");
}

#[test]
fn facade_server_rejects_oversized_prefers_reduced_transparency_request_head() {
  ...
  let server = rttp_server::server::HttpServer::bind("127.0.0.1:0")
    .expect("bind oversized Prefers-Reduced-Transparency server")
    .with_read_timeout(Some(Duration::from_secs(2)))
    .with_write_timeout(Some(Duration::from_secs(2)));
  let addr = server
    .local_addr()
    .expect("oversized Prefers-Reduced-Transparency server addr");
  let (observed_tx, observed_rx) = mpsc::channel::<()>();
  let handle = thread::spawn(move || {
    server
      .accept_one(|_request| {
  ...
  let request = format!(
    "GET /asset HTTP/1.1\r\nHost: example.test\r\nSec-CH-Prefers-Reduced-Transparency: {oversized}\r\nConnection: close\r\n\r\n"
  );
  let mut stream =
    TcpStream::connect(addr).expect("connect oversized Prefers-Reduced-Transparency");
  stream
    .write_all(request.as_bytes())
    .expect("write oversized Prefers-Reduced-Transparency request");
  ...
  assert!(
    ...
    "oversized Prefers-Reduced-Transparency must not reach the handler"
  );
  handle
    .join()
    .expect("oversized Prefers-Reduced-Transparency server thread");
}
...
#[test]
fn sync_client_and_server_exchange_canonical_sec_ch_viewport_width_metadata() {
  let server = rttp_server::server::HttpServer::bind("127.0.0.1:0")
    .expect("bind Sec-CH-Viewport-Width server");
  let addr = server
    .local_addr()
    .expect("Sec-CH-Viewport-Width server addr");
  let (observed_tx, observed_rx) = mpsc::channel();
  let handle = thread::spawn(move || {
    server
      .accept_one(|request| {
        ...
        observed_tx
          .send(observed)
          .expect("send observed Sec-CH-Viewport-Width metadata");
        HttpResponse::ok("OK")
      })
      .expect("serve Sec-CH-Viewport-Width metadata request");
  });
  ...
  assert_eq!(200, response.code());
  assert_eq!(
    (Some("1440".to_string()), Ok(Some("1440".to_string()))),
    observed_rx
      .recv()
      .expect("server should observe Sec-CH-Viewport-Width metadata")
  );
  handle.join().expect("Sec-CH-Viewport-Width server thread");
}

#[cfg(feature = "async")]
#[test]
fn async_client_and_server_exchange_canonical_sec_ch_viewport_width_metadata() {
  let server = rttp_server::server::HttpServer::bind("127.0.0.1:0")
    .expect("bind async Sec-CH-Viewport-Width server");
  let addr = server
    .local_addr()
    .expect("async Sec-CH-Viewport-Width server addr");
  let (observed_tx, observed_rx) = mpsc::channel();
  let handle = thread::spawn(move || {
    server
      .accept_one(|request| {
        ...
        observed_tx
          .send(observed)
          .expect("send observed async Sec-CH-Viewport-Width metadata");
        HttpResponse::ok("OK")
      })
      .expect("serve async Sec-CH-Viewport-Width metadata request");
  });
  ...
  assert_eq!(
    ...
    observed_rx
      .recv()
      .expect("server should observe async Sec-CH-Viewport-Width metadata")
  );
  handle
    .join()
    .expect("async Sec-CH-Viewport-Width server thread");
}
